...
use BlackCat\Database\Packages\DeletionJobs\Repository\DeletionJobRepositoryInterface;
...
class DeletionJobRepository implements DeletionJobRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'entity_table', 'entity_pk' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\DeletionJobs\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'reason', 'hard_delete', 'scheduled_at', 'started_at', 'finished_at', 'status', 'error', 'created_by' ]);
          return count($rows);
        }
