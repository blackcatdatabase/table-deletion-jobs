...
namespace BlackCat\Database\Packages\DeletionJobs\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\DeletionJobs\Definitions;
use BlackCat\Database\Packages\DeletionJobs\Criteria;
use BlackCat\Database\Packages\DeletionJobs\Dto\DeletionJobDto as Dto;
use BlackCat\Database\Packages\DeletionJobs\Mapper\DeletionJobDtoMapper as RowMapper;
...
final class DeletionJobRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\DeletionJobs\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'reason', 'hard_delete', 'scheduled_at', 'started_at', 'finished_at', 'status', 'error', 'created_by' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'entity_table', 'entity_pk' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\DeletionJobs\Definitions::table());
            return $bulk->upsertMany($rows, [ 'entity_table', 'entity_pk' ], [ 'reason', 'hard_delete', 'scheduled_at', 'started_at', 'finished_at', 'status', 'error', 'created_by' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DeletionJobs\Dto\DeletionJobDto|null */
    public function getByEntityTableAndEntityPk(string $entityTable, string $entityPk, bool $asDto = false): array|\BlackCat\Database\Packages\DeletionJobs\Dto\DeletionJobDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\DeletionJobs\Mapper\DeletionJobDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DeletionJobs\Dto\DeletionJobDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\DeletionJobs\Dto\DeletionJobDto|null {
