...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'entity_table', 'entity_pk', 'reason', 'hard_delete', 'scheduled_at', 'started_at', 'finished_at', 'status', 'error', 'created_by', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'entity_table', 'entity_pk', 'reason', 'error' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        $x = [ 'id', 'entity_table', 'entity_pk', 'reason', 'hard_delete', 'scheduled_at', 'started_at', 'finished_at', 'status', 'error', 'created_by', 'created_at' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\DeletionJobs\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\DeletionJobs\Definitions::softDeleteColumn();
