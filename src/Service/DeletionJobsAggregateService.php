...
namespace BlackCat\Database\Packages\DeletionJobs\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\DeletionJobs\Dto\DeletionJobDto;
use BlackCat\Database\Packages\DeletionJobs\Mapper\DeletionJobDtoMapper;
 ...
 */
final class DeletionJobsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private DeletionJobRepository $deletionJobRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'deletion_jobs' => $this->deletionJobRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\DeletionJobs\ModuleException("Record {$idStr} not found for locking.");
            }
